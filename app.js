...
// Modal elements for batch delete
...
// Modal elements for batch add tag
...
function applyFilters() {
  ...
  let filtered = allLinks.filter(
    (l) =>
      l.title.toLowerCase().includes(q) || (l.tags || "").toLowerCase().includes(q)
  );
...
function renderLinks(links) {
  ...
  // Batch Delete handler (open modal)
  deleteSelectedBtn.onclick = () => {
    batchDeleteMessage.textContent = `Are you sure you want to delete ${selectedIds.size} selected link(s)? This action cannot be undone.`;
  ...
  // Cancel batch delete modal
  batchDeleteCancelBtn.onclick = () => {
    batchDeleteModal.style.display = "none";
  };

  // Batch Add Tag handler (open modal)
  ...
  // Cancel add tag modal
  batchAddTagCancelBtn.onclick = () => {
    batchAddTagModal.style.display = "none";
  };
}
